...
public class DayMonthYearModel : INotifyPropertyChanged
{
    ...
    public DayMonthYearModel(PickerModelType displayType = PickerModelType.DayMonthYear)
    {
        ...
        wheelYear = new(Enumerable.Range(MinDate.Year, MaxDate.Year - MinDate.Year + 1).Reverse().Select(year => year.ToString()));
        wheelMonth = new(Enumerable.Range(1, 12).Select(month => CultureInfo.CurrentCulture.DateTimeFormat.GetMonthName(month)));
        wheelDay = new(Enumerable.Range(1, 31).Select(day => day.ToString()));
